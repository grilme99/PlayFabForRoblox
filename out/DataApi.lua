...
local GetObjects = TS.async(function(request)
	...
	local result = TS.await(MakeRequest('/Object/GetObjects', request, 'X-EntityToken', Settings.settings.entityToken):catch(function(e)
		error(e);
	end));
	return result;
end);
local SetObjects = TS.async(function(request)
	...
	local result = TS.await(MakeRequest('/Object/SetObjects', request, 'X-EntityToken', Settings.settings.entityToken):catch(function(e)
		error(e);
	end));
	return result;
end);
